            ...
            if(!alreadyBE) {
                ...
                if(bePips > 0 && profitPips >= bePips) {
                    ...
                    if(needModify) {
                        ...
                        // SELL: refresh position then try TP=0 first (many brokers reject Modify with TP)
                        if(!isBuy && position.SelectByTicket(ticket)) {
                            tpVal = position.TakeProfit();
                        }
                        bool modified = false;
                        if(isBuy) {
                            modified = trade.PositionModify(ticket, newSL, tpVal) || trade.PositionModify(ticket, newSL, 0);
                        } else {
                            // SELL: try (newSL, 0) then (newSL, tpVal) – order per Blueprint
                            modified = trade.PositionModify(ticket, newSL, 0) || trade.PositionModify(ticket, newSL, tpVal);
                        }
                        if(modified)
                            ...
                            if(trade.PositionModify(ticket, buf1, 0))
                                ...
                                Print("Fresh BE set (10 pip buffer) ", isBuy ? "BUY" : "SELL", " #", ticket);
                            else
                                Print("Fresh BE FAILED ", isBuy ? "BUY" : "SELL", " #", ticket, " err ", GetLastError(), " pips ", profitPips, " curSL ", currentSL, " want ", newSL);
                        }
                    }
                }
            }
        ...
        if(StringFind(position.Comment(), "_SCALP") >= 0) {
                ...
                if(!scalpAlreadyBE) {
                    ...
                    if(need) {
                        ...
                        bool ok = isBuy ? (trade.PositionModify(ticket, newSL, tpScalp) || trade.PositionModify(ticket, newSL, 0))
                                        : (trade.PositionModify(ticket, newSL, 0) || trade.PositionModify(ticket, newSL, tpScalp));
                        if(ok)
            ...
            if(UseBreakEven && profitPips >= REV_BEPips) {
                ...
                if(stillNoBE) {
                    ...
                    double b5 = isBuy ? NormalizeDouble(openPrice + 5.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 5.0 * pipValue, symbolDigits);
                    double b10 = isBuy ? NormalizeDouble(openPrice + 10.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 10.0 * pipValue, symbolDigits);
                    if(trade.PositionModify(ticket, ns, 0))
                ...
                if(!alreadyBE2) {
                    ...
                    if(need2) {
                        ...
                        bool okOther = isBuy ? (trade.PositionModify(ticket, newSL2, tpOther) || trade.PositionModify(ticket, newSL2, 0))
                                             : (trade.PositionModify(ticket, newSL2, 0) || trade.PositionModify(ticket, newSL2, tpOther));
                        if(okOther)
        ...
        if(UseBreakEven && profitPips >= REV_BEPips) {
            ...
            if(stillNoBE) {
                ...
                bool okEnd = isBuy ? (trade.PositionModify(ticket, newSLEnd, tpEnd) || trade.PositionModify(ticket, newSLEnd, 0))
                                   : (trade.PositionModify(ticket, newSLEnd, 0) || trade.PositionModify(ticket, newSLEnd, tpEnd));
                if(okEnd) {
