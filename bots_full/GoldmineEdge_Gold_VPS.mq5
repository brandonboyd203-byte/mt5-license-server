...
bool ValidateLicenseRemote() {
    ...
    if(res != 200) {
        Print("ERROR: License server returned HTTP ", res, " (expected 200). Response: ", response);
        return false;
    }
...
int OnInit() {
    ...
    for(int ai = PositionsTotal() - 1; ai >= 0; ai--) {
        ...
        if(idx >= ArraySize(tp1Hit)) {
            int oldSize = ArraySize(tp1Hit);
            int ns = idx + 20;
            ArrayResize(tp1Hit, ns);
            ArrayResize(tp2Hit, ns);
            ArrayResize(tp3Hit, ns);
            ArrayResize(tp4Hit, ns);
    ...
    if(adopted > 0) Print("Goldmine Edge VPS: Adopted ", adopted, " existing position(s) for BE/TP management.");
