//+------------------------------------------------------------------+
//|                         GoldmineNexus_Gold_VPS.mq5                 |
//|     VPS: Restart-safe. Adopts existing positions on init.         |
//|     Goldmine Nexus - Gold (XAUUSD) | FVG + OB + Session sweeps    |
//+------------------------------------------------------------------+
...
input double RiskPercent = 4.0;               // Risk per trade (%) - fallback when not layering
input double FirstTradeRisk = 4.0;           // Risk for FIRST trade (%) - fallback
...
#ifndef PLUG_SYMBOL_GOLD
input double BreakEvenPips_Silver = 25.0;    // Move to BE at this many pips (Silver)
#endif
...
input bool FVG_RequirePullback = false;       // Relaxed: allow touch entries (more trades).
...
input bool RequireEngulfingForOB = false;      // Relaxed: OB entries can trigger without M1/M3 engulfing
...
input bool RequireEngulfingAtSupport = false;  // Relaxed: allow support/resistance entries without engulfing
...
input bool SessionSweep_RequireSweptFirst = false; // Relaxed: allow session retest entries without prior strict sweep state
...
input double Breakout_SL_Pips = 20.0;         // SL (pips) beyond broken level for breakout trades
...
// Re-entry after BE: zone + direction + time (set in OnTradeTransaction when position closes at BE)
double g_BEClosedEntryPrice = 0;
int    g_BEClosedDirection = 0;   // 1 = BUY, -1 = SELL
datetime g_BEClosedTime = 0;
bool   g_BEClosedUsed = false;     // true after we used it for one re-entry
string g_BEClosedSymbol = "";
bool   g_BEClosedWasBreakout = false;         // true when the BE-closed position was a breakout trade
...
//+------------------------------------------------------------------+
bool CheckLicense() {
    Print("Nexus Gold VPS runtime: license checks bypassed for VDS deployment.");
    return true;
}
...
int OnInit() {
    ...
    for(int ai = PositionsTotal() - 1; ai >= 0; ai--) {
        ...
        if(profitPips >= TP6_Pips) tp6Hit[idx] = true;
        adopted++;
    }
    if(adopted > 0) Print("Goldmine Nexus VPS: Adopted ", adopted, " existing position(s) for BE/TP management.");
...
//+------------------------------------------------------------------+
//| Detect position closed at/near BE and store zone for re-entry    |
//+------------------------------------------------------------------+
void OnTradeTransaction(const MqlTradeTransaction& trans, const MqlTradeRequest& request, const MqlTradeResult& result) {
    ...
    if(diffPips > 5.0) return; // closed not at BE (more than 5 pips away)
    ...
    Print("*** RE-ENTRY: Position closed at BE (", DoubleToString(diffPips, 1), " pips) | Entry was ", entryPrice, (g_BEClosedWasBreakout ? " | BO" : ""), " | Re-entry allowed in zone for ", ReentryAfterBE_MaxMinutes, " min ***");
}
        ...
        if(orderBlocks[i].isBullish) {
            ...
            if(priceInZone || priceNearZone || isHighProbBullish) {
                ...
                if(shouldEnter) {
                    ...
                    // Re-entry after BE: same zone, confluence still there, use reduced risk
...
bool CheckBreakoutEntries(double ask, int buyPositions, int sellPositions, bool globalBlockBUY, bool globalBlockSELL) {
    ...
    // Range levels: when London not set (or use range), breakouts can trigger before London
    ...
    // SELL breakout: last closed bar broke below level (London low or range low when London not set)
    ...
    // BUY breakout: last closed bar broke above level (London high or range high when London not set)
...
double GetPipValueForSymbol(string sym) {
    ...
    if(StringFind(s, "XAU") >= 0 || StringFind(s, "GOLD") >= 0) {
        ...
        return (d >= 3) ? 0.01 : 0.1;  // 3-digit XAU = 1 pip 0.01; 2-digit = 0.1 (TP1 at 50 pips consistent)
    }
...
//+------------------------------------------------------------------+
//| Get recent swing high (resistance) for structure-based trail – SELL: SL above this |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Get recent swing low (support) for structure-based trail – BUY: SL below this |
//+------------------------------------------------------------------+
...
void ManagePositions() {
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        // Gold: respect 2 vs 3 digit quote (3-digit = 1 pip 0.01 so TP1 at 50 pips is consistent)
        bool isGoldSym = (StringFind(posSymbolUpper, "XAU") >= 0 || StringFind(posSymbolUpper, "GOLD") >= 0);
        double goldPipSize = 0.1;
        if(isGoldSym) goldPipSize = (posDigits >= 3) ? 0.01 : 0.1;
        if(isGoldSym && posType == POSITION_TYPE_BUY)
            currentProfitPips = (currentBID - openPrice) / goldPipSize;
        if(isGoldSym && posType == POSITION_TYPE_SELL)
            currentProfitPips = (openPrice - currentASK) / goldPipSize;
        // Silver: 1 pip = 0.01
        if(isSilver) {
            if(posType == POSITION_TYPE_BUY)
...
#ifdef PLUG_SYMBOL_GOLD
        // GOLD-ONLY EA: pip from broker digits (3-digit XAU = 0.01, 2-digit = 0.1) so TP1 fires at 50 pips
        posPipValue = goldPipSize;
        isSilver = false;
        isGoldSym = true;
        currentProfitPips = (posType == POSITION_TYPE_BUY) ? (currentBID - openPrice) / goldPipSize : (openPrice - currentASK) / goldPipSize;
#endif
#ifdef PLUG_SYMBOL_SILVER
        ...
        if(posType == POSITION_TYPE_SELL)
            currentProfitPips = (openPrice - currentASK) / 0.01;
#endif
        // SELL profit: always use fixed pip size so BE/TP never fail (Silver=0.01, Gold=goldPipSize)
        if(posType == POSITION_TYPE_SELL)
            currentProfitPips = (openPrice - currentASK) / (isSilver ? 0.01 : goldPipSize);
...
#endif
        // GOLD SELL FIX: Force BE trigger from price distance (pip = goldPipSize) so BE always fires (like Goldmine Edge / SMC)
        ...
        if(isGold && posType == POSITION_TYPE_SELL) {
            ...
            double pipsFromPrice = priceProfit / goldPipSize;
        ...
        if(isGold && posType == POSITION_TYPE_SELL) {
            ...
            double pipsFromPriceSELL = priceProfitSELL / goldPipSize;
        ...
        // CRITICAL FIX FOR SELL TRADES: Use fixed pip size so BE/TP never fail (Gold=goldPipSize, Silver=0.01)
        if(posType == POSITION_TYPE_SELL) {
        ...
        if(currentProfitPips > 0) {
            ...
            // CRITICAL: Log high-profit trades that aren't triggering TP (Gold SELL was taking first profit at 100 pips - fix pip size)
            if(currentProfitPips >= 20 && partialCloseLevel[ticketIndex] == 0 && (isSilver || (isGoldSym && posType == POSITION_TYPE_SELL))) {
                ...
                if(TimeCurrent() - lastHighProfitLog >= 3) {
                    ...
                    Print("  Ticket: ", ticket, " | tp1Hit: ", tp1Hit[ticketIndex], " | currentLevel: ", partialCloseLevel[ticketIndex], (isGoldSym ? " | Gold pipSize: " + DoubleToString(goldPipSize, 2) : ""));
            ...
            if(currentLevel == 0 && (currentProfitPips >= TP1_Pips || tp1Pending[ticketIndex]) && tp1Hit[ticketIndex] && canCloseMore) {
                ...
                if(closeVolume >= minLot && remainingVolume >= minLot) {
                    ...
                    for(int retry = 0; retry < 3; retry++) {
                        ...
                        if(ExecuteVerifiedPartialClose(ticket, posSymbol, closeVolume, currentVolume, lotStep, observedVolume, retcode, errorDesc, lastErr)) {
                            partialCloseLevel[ticketIndex] = 1;
                            tp1Pending[ticketIndex] = false;
        ...
        // Step 5b: Trail SL when profit >= TrailStartPips – structure-based (above swing high for SELL, below swing low for BUY) or fixed distance
