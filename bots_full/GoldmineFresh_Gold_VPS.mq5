//+------------------------------------------------------------------+
//|                                         GoldmineFresh_Gold_VPS.mq5 |
//|     VPS: Restart-safe. Adopts existing positions on init.         |
//|     Goldmine Fresh - Gold. OB, session, TL, FVG. 1 pip = 0.1.     |
//+------------------------------------------------------------------+
...
input double Partial50AtPips = 100.0; // At this many pips: close 50%, set BE on rest; 0 = use TP1-TP4 only
...
input int    Breakout_MinRetestSeconds = 90;   // Minimum wait after breakout before retest entry (prevents rushed entries)
input double Breakout_MinImpulsePips = 8.0;    // Price must travel this far beyond breakout before retest is valid
...
input double REV_BEPips = 25.0;               // BE at this many pips for _REV (when UseBreakEven)
input double REV_Partial50AtPips = 100.0;     // _REV: close 50% at this many pips, set BE on rest; 0 = use general Partial50AtPips
...
bool       did50Partial[];  // true = we closed 50% at Partial50AtPips; rest runs with BE + trail/setup TP only
double     origVolume[];
int        partialLevel[];
...
static datetime lastBuyEntryTime = 0, lastSellEntryTime = 0;
...
bool ValidateLicenseRemote() {
    ...
    if(StringFind(resp, "\"valid\":true") >= 0) return true;   // no log spam; CheckLicense() caches result
    Print("License: invalid");
    return false;
}

// Cache license result so we don't spam server and log every tick
static bool   _licenseCached = false;
static bool   _licenseValid  = false;
static datetime _licenseCheckedAt = 0;
#define LICENSE_RECHECK_SEC 3600   // Re-validate at most once per hour
...
bool CheckLicense() {
    ...
    if(_licenseCached && (TimeCurrent() - _licenseCheckedAt) < LICENSE_RECHECK_SEC)
        return _licenseValid;
    for(int t = 0; t < 3; t++) {
        if(ValidateLicenseRemote()) {
            _licenseCached = true;
            _licenseValid  = true;
            _licenseCheckedAt = TimeCurrent();
            return true;
        }
        Sleep(500);
    }
...
bool OpenBuyEx(double slPrice, string setupSuffix, bool asScalp = false) {
    ...
    if(MinSecondsBetweenSameDirectionEntries > 0 && lastBuyEntryTime > 0 && (now - lastBuyEntryTime) < MinSecondsBetweenSameDirectionEntries) {
        Print("Fresh BUY blocked by cooldown: ", (int)(MinSecondsBetweenSameDirectionEntries - (now - lastBuyEntryTime)), "s remaining");
        return false;
    }
    ...
    for(int attempt = 1; attempt <= 3; attempt++) {
        ...
        if(trade.Buy(lot, _Symbol, ask, sl, 0, cmt)) {
            lastBuyEntryTime = now;
            Print("Fresh BUY ", setupSuffix, (asScalp ? " SCALP" : ""), " @ ", ask, " SL ", sl, " lots ", lot, " (attempt ", attempt, ")");
            return true;
        }
        int err = GetLastError();
        Print("Fresh BUY send failed (attempt ", attempt, ") err ", err, " lots ", lot, " sl ", sl);
        if(attempt < 3) Sleep(250 * attempt);
    }
    return false;
}
...
bool OpenSellEx(double slPrice, string setupSuffix, bool asScalp = false) {
    datetime now = TimeCurrent();
    if(MinSecondsBetweenSameDirectionEntries > 0 && lastSellEntryTime > 0 && (now - lastSellEntryTime) < MinSecondsBetweenSameDirectionEntries) {
        Print("Fresh SELL blocked by cooldown: ", (int)(MinSecondsBetweenSameDirectionEntries - (now - lastSellEntryTime)), "s remaining");
        return false;
    }
    ...
    for(int attempt = 1; attempt <= 3; attempt++) {
        ...
        if(trade.Sell(lot, _Symbol, bid, sl, 0, cmt)) {
            lastSellEntryTime = now;
            Print("Fresh SELL ", setupSuffix, (asScalp ? " SCALP" : ""), " @ ", bid, " SL ", sl, " lots ", lot, " (attempt ", attempt, ")");
            return true;
        }
        int err = GetLastError();
        Print("Fresh SELL send failed (attempt ", attempt, ") err ", err, " lots ", lot, " sl ", sl);
        if(attempt < 3) Sleep(250 * attempt);
    }
    return false;
}
...
void TryEntries() {
    ...
    if(UseEntry_SessionBreakout && UseSessionLevels) {
        ...
        static datetime brokeAboveAt = 0, brokeBelowAt = 0;
        static bool impulseAboveDone = false, impulseBelowDone = false;

        if(breakoutHigh > 0 && ask > breakoutHigh + retestTol) {
            if(!brokeAboveSession) {
                brokeAboveAt = TimeCurrent();
                impulseAboveDone = false;
            }
            brokeAboveSession = true;
            if(ask >= breakoutHigh + Breakout_MinImpulsePips * pipValue) impulseAboveDone = true;
        }
        if(breakoutLow > 0 && bid < breakoutLow - retestTol) {
            if(!brokeBelowSession) {
                brokeBelowAt = TimeCurrent();
                impulseBelowDone = false;
            }
            brokeBelowSession = true;
            if(bid <= breakoutLow - Breakout_MinImpulsePips * pipValue) impulseBelowDone = true;
        }

        bool waitAboveDone = (Breakout_MinRetestSeconds <= 0) || (brokeAboveAt > 0 && (TimeCurrent() - brokeAboveAt) >= Breakout_MinRetestSeconds);
        bool waitBelowDone = (Breakout_MinRetestSeconds <= 0) || (brokeBelowAt > 0 && (TimeCurrent() - brokeBelowAt) >= Breakout_MinRetestSeconds);

        if(breakoutHigh > 0 && buys < MaxPositionsPerSide && bosOkBuy && brokeAboveSession && waitAboveDone && impulseAboveDone &&
           ask >= breakoutHigh - retestTol && ask <= breakoutHigh + retestTol && SessionSweepHasBullishEngulfing()) {
            ...
            if(OpenBuyEx(sl, "_BO", thisBuyIsScalp)) { brokeAboveSession = false; brokeAboveAt = 0; impulseAboveDone = false; return; }
        }
        if(breakoutLow > 0 && sells < MaxPositionsPerSide && bosOkSell && brokeBelowSession && waitBelowDone && impulseBelowDone &&
           bid >= breakoutLow - retestTol && bid <= breakoutLow + retestTol && SessionSweepHasBearishEngulfing()) {
            ...
            if(OpenSellEx(sl, "_BO", thisSellIsScalp)) { brokeBelowSession = false; brokeBelowAt = 0; impulseBelowDone = false; return; }
        }
        if(breakoutHigh > 0 && ask < breakoutHigh - retestTol * 2) { brokeAboveSession = false; brokeAboveAt = 0; impulseAboveDone = false; }
        if(breakoutLow > 0 && bid > breakoutLow + retestTol * 2) { brokeBelowSession = false; brokeBelowAt = 0; impulseBelowDone = false; }
    }
...
//| All Fresh trades get BE + 50% partial except _SCALP (opposite).   |
//| _SCALP only: ScalpBE_Pips, ScalpPartial_Pips, ScalpTP_Pips.         |
//| All others: BreakEvenPips (REV_BEPips for _REV) + Partial50AtPips (REV_Partial50AtPips for _REV). |
//+------------------------------------------------------------------+
void ManagePositions() {
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if(UseBreakEven && profitPips > 0) {
            double beTolerance = 2.0 * pipValue;  // consider BE "set" if SL within 2 pips of entry
            bool alreadyBE = isBuy ? (currentSL > 0 && currentSL >= openPrice - beTolerance) : (currentSL > 0 && currentSL <= openPrice + beTolerance);
            if(!alreadyBE) {
                string cmtBE = position.Comment();
                // REV SCALP (both _REV and _SCALP): use REV_BEPips (50) so opposite-direction reversal gets BE at 50
                bool hasREV = (StringFind(cmtBE, "_REV") >= 0);
                bool hasSCALP = (StringFind(cmtBE, "_SCALP") >= 0);
                double bePips = (hasREV && hasSCALP) ? REV_BEPips : (hasSCALP ? ScalpBE_Pips : (hasREV ? REV_BEPips : BreakEvenPips));
                if(bePips > 0 && profitPips >= bePips) {
                    if(!isBuy) Print("[SELL_BE_ARM] ticket=", ticket, " pips=", profitPips, " trigger=", bePips);
                    double newSL = NormalizeDouble(openPrice, symbolDigits);
                    // SELL: need modify when SL above entry or no SL. BUY: when SL below entry or no SL.
                    bool needModify = isBuy ? (currentSL < newSL - point || currentSL == 0) : (currentSL > newSL + point || currentSL == 0);
                    if(needModify) {
                        double tpVal = position.TakeProfit();
                        if(!isBuy && position.SelectByTicket(ticket)) tpVal = position.TakeProfit();
                        bool modified = false;
                        if(isBuy) {
                            modified = trade.PositionModify(ticket, newSL, tpVal) || trade.PositionModify(ticket, newSL, 0);
                        } else {
                            // SELL: try preserve-TP first (some brokers reject TP=0), then try cushion (1 pip in profit)
                            modified = trade.PositionModify(ticket, newSL, tpVal) || trade.PositionModify(ticket, newSL, 0);
                            if(!modified) {
                                double cushionSL = NormalizeDouble(openPrice - pipValue, symbolDigits); // 1 pip in profit
                                modified = trade.PositionModify(ticket, cushionSL, tpVal) || trade.PositionModify(ticket, cushionSL, 0);
                            }
                        }
                        if(modified)
                            Print(isBuy ? "Fresh BE set BUY #" : "[SELL_BE_MOVE_OK] Fresh BE set SELL #", ticket);
                        else {
                            // Buffers: BUY = above entry; SELL = below entry (lock profit)
                            double buf1 = isBuy ? NormalizeDouble(openPrice + pipValue, symbolDigits) : NormalizeDouble(openPrice - pipValue, symbolDigits);
                            double buf5 = isBuy ? NormalizeDouble(openPrice + 5.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 5.0 * pipValue, symbolDigits);
                            double buf10 = isBuy ? NormalizeDouble(openPrice + 10.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 10.0 * pipValue, symbolDigits);
                            if(trade.PositionModify(ticket, buf1, tpVal))
                                Print(isBuy ? "Fresh BE set (1 pip buffer) BUY #" : "[SELL_BE_MOVE_OK] Fresh BE set (1 pip buffer) SELL #", ticket);
                            else if(trade.PositionModify(ticket, buf1, 0))
                                Print(isBuy ? "Fresh BE set (1 pip buffer) BUY #" : "[SELL_BE_MOVE_OK] Fresh BE set (1 pip buffer) SELL #", ticket);
                            else if(trade.PositionModify(ticket, buf5, 0))
                                Print(isBuy ? "Fresh BE set (5 pip buffer) BUY #" : "[SELL_BE_MOVE_OK] Fresh BE set (5 pip buffer) SELL #", ticket);
                            else if(trade.PositionModify(ticket, buf10, 0))
                                Print(isBuy ? "Fresh BE set (10 pip buffer) BUY #" : "[SELL_BE_MOVE_OK] Fresh BE set (10 pip buffer) SELL #", ticket);
                            else
                                Print(isBuy ? "Fresh BE FAILED BUY #" : "[SELL_BE_MOVE_FAIL] Fresh BE FAILED SELL #", ticket, " err ", GetLastError(), " pips ", profitPips, " curSL ", currentSL, " want ", newSL);
                        }
                    }
                }
            }
        ...
        // --- _REV only: 50% partial at REV_Partial50AtPips, BE on rest (guaranteed for reversal trades) ---
        if(StringFind(position.Comment(), "_REV") >= 0 && REV_Partial50AtPips > 0 && idx < ArraySize(did50Partial) && !did50Partial[idx] && level == 0 && profitPips >= REV_Partial50AtPips) {
            double closeVolRev = MathFloor(currentVol * 0.5 / lotStep) * lotStep;
            if(closeVolRev < minLot) closeVolRev = minLot;
            if(closeVolRev >= currentVol - lotStep * 0.5) closeVolRev = currentVol - minLot;
            if(closeVolRev >= minLot && closeVolRev < currentVol) {
                if(trade.PositionClosePartial(ticket, closeVolRev)) {
                    did50Partial[idx] = true;
                    Print("Fresh REV 50% partial #", ticket, " at ", profitPips, " pips (", closeVolRev, " lots); rest runs with BE");
                    continue;
                }
                if(trade.PositionClose(ticket)) {
                    did50Partial[idx] = true;
                    Print("Fresh REV 50% partial failed, full close #", ticket, " err ", GetLastError());
                    continue;
                }
            } else if(closeVolRev >= currentVol - lotStep * 0.5) {
                if(trade.PositionClose(ticket)) {
                    did50Partial[idx] = true;
                    Print("Fresh REV 50% full close (min lot) #", ticket);
                    continue;
                }
            }
        }
        if(StringFind(position.Comment(), "_SCALP") >= 0) {
            // --- _SCALP: set BE (REV SCALP uses REV_BEPips=50, pure scalp uses ScalpBE_Pips) ---
            string cmtScalp = position.Comment();
            bool revScalp = (StringFind(cmtScalp, "_REV") >= 0);
            double scalpBEPips = revScalp ? REV_BEPips : ScalpBE_Pips;
            if(UseBreakEven && scalpBEPips > 0 && profitPips >= scalpBEPips) {
                double beTol = 2.0 * pipValue;
                bool scalpAlreadyBE = isBuy ? (currentSL > 0 && currentSL >= openPrice - beTol) : (currentSL > 0 && currentSL <= openPrice + beTol);
                if(!scalpAlreadyBE) {
                    double newSL = NormalizeDouble(openPrice, symbolDigits);
                    bool need = isBuy ? (currentSL < newSL - point || currentSL == 0) : (currentSL > newSL + point || currentSL == 0);
                    if(need) {
                        double tpScalp = position.TakeProfit();
                        bool ok = isBuy ? (trade.PositionModify(ticket, newSL, tpScalp) || trade.PositionModify(ticket, newSL, 0))
                                        : (trade.PositionModify(ticket, newSL, tpScalp) || trade.PositionModify(ticket, newSL, 0));
                        if(!ok && !isBuy) {
                            double cushionSL = NormalizeDouble(openPrice - pipValue, symbolDigits);
                            ok = trade.PositionModify(ticket, cushionSL, tpScalp) || trade.PositionModify(ticket, cushionSL, 0);
                        }
                        if(ok)
                            Print(revScalp ? "Fresh REV SCALP BE set #" : "Fresh SCALP BE set #", ticket);
                        else {
                            double b1 = isBuy ? NormalizeDouble(openPrice + pipValue, symbolDigits) : NormalizeDouble(openPrice - pipValue, symbolDigits);
                            double b5 = isBuy ? NormalizeDouble(openPrice + 5.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 5.0 * pipValue, symbolDigits);
                            double b10 = isBuy ? NormalizeDouble(openPrice + 10.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 10.0 * pipValue, symbolDigits);
                            if(trade.PositionModify(ticket, b1, 0))
                                Print(revScalp ? "Fresh REV SCALP BE set (1 pip buffer) #" : "Fresh SCALP BE set (1 pip buffer) #", ticket);
                            else if(trade.PositionModify(ticket, b5, 0))
                                Print(revScalp ? "Fresh REV SCALP BE set (5 pip buffer) #" : "Fresh SCALP BE set (5 pip buffer) #", ticket);
                            else if(trade.PositionModify(ticket, b10, 0))
                                Print(revScalp ? "Fresh REV SCALP BE set (10 pip buffer) #" : "Fresh SCALP BE set (10 pip buffer) #", ticket);
                            else
                                Print(revScalp ? "Fresh REV SCALP BE FAILED #" : "Fresh SCALP BE FAILED #", ticket, " err ", GetLastError());
                        }
                    }
                }
            }
            ...
            if(ScalpTP_Pips > 0 && profitPips >= ScalpTP_Pips * 0.95) {
                if(trade.PositionClose(ticket)) { Print(isBuy ? "Fresh scalp TP BUY #" : "[SELL_TP_SET] Fresh scalp TP SELL #", ticket, " ", profitPips, " pips"); continue; }
            }
            // --- _SCALP catch-all: we never reach the global catch-all (we continue above), so try BE here if still not set ---
            if(UseBreakEven && profitPips >= REV_BEPips) {
                double beTolS = 2.0 * pipValue;
                bool stillNoBE = isBuy ? (currentSL < openPrice - beTolS || currentSL == 0) : (currentSL > openPrice + beTolS || currentSL == 0);
                if(stillNoBE) {
                    double ns = NormalizeDouble(openPrice, symbolDigits);
                    double b1 = isBuy ? NormalizeDouble(openPrice + pipValue, symbolDigits) : NormalizeDouble(openPrice - pipValue, symbolDigits);
                    double b5 = isBuy ? NormalizeDouble(openPrice + 5.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 5.0 * pipValue, symbolDigits);
                    double b10 = isBuy ? NormalizeDouble(openPrice + 10.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 10.0 * pipValue, symbolDigits);
                    bool okCatch = trade.PositionModify(ticket, ns, 0) || trade.PositionModify(ticket, ns, position.TakeProfit());
                    if(!okCatch && !isBuy) {
                        double cushionSL = NormalizeDouble(openPrice - pipValue, symbolDigits);
                        okCatch = trade.PositionModify(ticket, cushionSL, 0) || trade.PositionModify(ticket, cushionSL, position.TakeProfit());
                    }
                    if(okCatch)
                        Print("Fresh SCALP catch-all BE set #", ticket);
                    else if(trade.PositionModify(ticket, b1, 0))
                        Print("Fresh SCALP catch-all BE (1 pip) #", ticket);
                    else if(trade.PositionModify(ticket, b5, 0))
                        Print("Fresh SCALP catch-all BE (5 pip) #", ticket);
                    else if(trade.PositionModify(ticket, b10, 0))
                        Print("Fresh SCALP catch-all BE (10 pip) #", ticket);
                    else
                        Print("Fresh SCALP catch-all BE FAILED #", ticket, " err ", GetLastError(), " pips ", profitPips);
                }
            }
            continue;
        }
        // --- All other Fresh trades (_OB, _SS, _DC, _TL_BR, _TL_OB, _HK, _BO): BE + 50% partial (same feature; only _SCALP uses ScalpBE/ScalpPartial/ScalpTP) ---
        if(UseBreakEven && profitPips > 0) {
            string cmtOther = position.Comment();
            double bePipsOther = (StringFind(cmtOther, "_REV") >= 0) ? REV_BEPips : BreakEvenPips;
            if(bePipsOther > 0 && profitPips >= bePipsOther) {
                double beTol2 = 2.0 * pipValue;
                bool alreadyBE2 = isBuy ? (currentSL > 0 && currentSL >= openPrice - beTol2) : (currentSL > 0 && currentSL <= openPrice + beTol2);
                if(!alreadyBE2) {
                    double newSL2 = NormalizeDouble(openPrice, symbolDigits);
                    bool need2 = isBuy ? (currentSL < newSL2 - point || currentSL == 0) : (currentSL > newSL2 + point || currentSL == 0);
                    if(need2) {
                        double tpOther = position.TakeProfit();
                        bool okOther = isBuy ? (trade.PositionModify(ticket, newSL2, tpOther) || trade.PositionModify(ticket, newSL2, 0))
                                             : (trade.PositionModify(ticket, newSL2, tpOther) || trade.PositionModify(ticket, newSL2, 0));
                        if(!okOther && !isBuy) {
                            double cushionSL = NormalizeDouble(openPrice - pipValue, symbolDigits);
                            okOther = trade.PositionModify(ticket, cushionSL, tpOther) || trade.PositionModify(ticket, cushionSL, 0);
                        }
                        if(okOther)
                            Print("Fresh BE set (all) ", isBuy ? "BUY" : "SELL", " #", ticket);
                        else {
                            double buf1 = isBuy ? NormalizeDouble(openPrice + pipValue, symbolDigits) : NormalizeDouble(openPrice - pipValue, symbolDigits);
                            double buf5 = isBuy ? NormalizeDouble(openPrice + 5.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 5.0 * pipValue, symbolDigits);
                            if(trade.PositionModify(ticket, buf1, 0))
                                Print("Fresh BE set (1 pip buffer) ", isBuy ? "BUY" : "SELL", " #", ticket);
                            else if(trade.PositionModify(ticket, buf5, 0))
                                Print("Fresh BE set (5 pip buffer) ", isBuy ? "BUY" : "SELL", " #", ticket);
                            else
                                Print("Fresh BE FAILED #", ticket, " err ", GetLastError());
                        }
                    }
                }
            }
        ...
        // --- 50% partial at Partial50AtPips: close half, set BE on rest, leave runner (no TP1–TP4 ladder) ---
        if(Partial50AtPips > 0 && idx < ArraySize(did50Partial) && !did50Partial[idx] && level == 0 && profitPips >= Partial50AtPips) {
            double closeVol = MathFloor(currentVol * 0.5 / lotStep) * lotStep;
            if(closeVol < minLot) closeVol = minLot;
            if(closeVol >= currentVol - lotStep * 0.5) closeVol = currentVol - minLot;
            if(closeVol >= minLot && closeVol < currentVol) {
                if(trade.PositionClosePartial(ticket, closeVol)) {
                    did50Partial[idx] = true;
                    Print("Fresh 50% partial #", ticket, " at ", profitPips, " pips (", closeVol, " lots); rest runs with BE");
                    // Set BE on the remaining position on next tick (position object is still old vol); we'll set BE when we see it again
                    continue;
                }
            } else if(closeVol >= currentVol - lotStep * 0.5) {
                if(trade.PositionClose(ticket)) {
                    did50Partial[idx] = true;
                    Print("Fresh 50% full close (min lot) #", ticket);
                    continue;
                }
            }
        }
            ...
            if(targetPrice > 0) {
                ...
                if(targetPips > 5) {
                    ...
                    if(profitPips >= effectiveTarget * 0.95) {
                        if(trade.PositionClose(ticket))
                            Print(isBuy ? "Fresh setup-based TP BUY #" : "[SELL_TP_SET] Fresh setup-based TP SELL #", ticket, " ", profitPips, " pips (cap ", effectiveTarget, ")");
                        continue;
                    }
                }
            }
        ...
        // --- TP1–TP4 partials: skip if we did 50% partial (runner only); else wait for Partial50AtPips before taking TP1 ---
        if(idx < ArraySize(did50Partial) && did50Partial[idx]) continue;
        if(profitPips <= 0) continue;
        if(Partial50AtPips > 0 && !did50Partial[idx] && profitPips < Partial50AtPips) continue;  // wait for 50% partial first
        ...
        // --- Catch-all BE: if 50+ pips profit and BE still not set (e.g. comment truncated, or Modify failed earlier), try again ---
        if(UseBreakEven && profitPips >= REV_BEPips) {
            double beTolEnd = 2.0 * pipValue;
            bool stillNoBE = isBuy ? (currentSL < openPrice - beTolEnd || currentSL == 0) : (currentSL > openPrice + beTolEnd || currentSL == 0);
            if(stillNoBE) {
                double newSLEnd = NormalizeDouble(openPrice, symbolDigits);
                double tpEnd = position.TakeProfit();
                double b1 = isBuy ? NormalizeDouble(openPrice + pipValue, symbolDigits) : NormalizeDouble(openPrice - pipValue, symbolDigits);
                double b5 = isBuy ? NormalizeDouble(openPrice + 5.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 5.0 * pipValue, symbolDigits);
                double b10 = isBuy ? NormalizeDouble(openPrice + 10.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 10.0 * pipValue, symbolDigits);
                bool okEnd = isBuy ? (trade.PositionModify(ticket, newSLEnd, tpEnd) || trade.PositionModify(ticket, newSLEnd, 0))
                                   : (trade.PositionModify(ticket, newSLEnd, tpEnd) || trade.PositionModify(ticket, newSLEnd, 0));
                if(!okEnd && !isBuy) {
                    double cushionSL = NormalizeDouble(openPrice - pipValue, symbolDigits);
                    okEnd = trade.PositionModify(ticket, cushionSL, tpEnd) || trade.PositionModify(ticket, cushionSL, 0);
                }
                if(okEnd) {
                    Print("Fresh BE catch-all set #", ticket, " ", profitPips, " pips");
                } else if(trade.PositionModify(ticket, b1, 0))
                    Print("Fresh BE catch-all (1 pip buffer) #", ticket);
                else if(trade.PositionModify(ticket, b5, 0))
                    Print("Fresh BE catch-all (5 pip buffer) #", ticket);
                else if(trade.PositionModify(ticket, b10, 0))
                    Print("Fresh BE catch-all (10 pip buffer) #", ticket);
                else
                    Print("Fresh BE catch-all FAILED #", ticket, " err ", GetLastError(), " pips ", profitPips);
            }
        }
...
void AdoptExistingPositions_VPS() {
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        bool commentOk = (MagicNumber == 0 && StringFind(position.Comment(), "Goldmine") >= 0);
        ...
        double profitPips = (position.Type() == POSITION_TYPE_BUY) ? (currentPrice - openPrice) / pipValue : (openPrice - currentPrice) / pipValue;
        int idx = GetOrCreateTicketIndex(ticket);
        if(idx >= ArraySize(origVolume) || origVolume[idx] <= 0) origVolume[idx] = vol;
        if(profitPips >= TP1_Pips) tp1Hit[idx] = true;
        if(profitPips >= TP2_Pips) tp2Hit[idx] = true;
        if(profitPips >= TP3_Pips) tp3Hit[idx] = true;
        if(profitPips >= TP4_Pips) tp4Hit[idx] = true;
        if(profitPips >= Partial50AtPips) did50Partial[idx] = true;
        adopted++;
    }
    if(adopted > 0) Print("Goldmine Fresh VPS: Adopted ", adopted, " existing position(s) for BE/TP management.");
}
...
int OnInit() {
    ...
    AdoptExistingPositions_VPS();
    if(!CheckLicense()) return INIT_FAILED;
    Print("Goldmine Fresh v3: BE=", UseBreakEven, " BE@", BreakEvenPips, " REV BE@", REV_BEPips, " REV50@", REV_Partial50AtPips, " Partial50@", Partial50AtPips, " TP1-4 ", TP1_Pips, "/", TP2_Pips, "/", TP3_Pips, "/", TP4_Pips, " Cap=", MaxSetupTargetPips, " pips. Risk ", RiskPercent, "%");
