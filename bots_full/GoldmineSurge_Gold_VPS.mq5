...
input double MinOppositeDistancePips = 5.0;  // Min distance from opposite trades (pips) - 0 = disabled
...
int OnInit() {
    ...
    for(int ai = PositionsTotal() - 1; ai >= 0; ai--) {
        ...
        int idx = (int)(ticket % 10000);
        if(idx >= ArraySize(beHit)) { int ns = idx + 10; ArrayResize(beHit, ns); ArrayResize(originalVolume, ns); ArrayResize(partialCloseHit, ns); for(int j = ArraySize(beHit)-10; j < ns; j++) { beHit[j]=false; originalVolume[j]=0; partialCloseHit[j]=0; } }
        if(originalVolume[idx] == 0) originalVolume[idx] = vol;
        if(profitPips >= BreakEvenPips) beHit[idx] = true;
        if(profitPips >= PartialClosePips) partialCloseHit[idx] = 1;
        adopted++;
    }
    if(adopted > 0) Print("Goldmine Surge VPS: Adopted ", adopted, " existing position(s) for BE/TP management.");
    ...
    for(int i = 0; i < ArraySize(orderBlocks); i++) {
        ...
        if(!orderBlocks[i].isBullish) {
            ...
            if(inZone && sellPositions < MaxEntries) {
                ...
                // Check momentum
                if(RequireMomentum && !HasMomentum(false)) {
                    Print("BEARISH OB RETEST BLOCKED: Insufficient momentum");
                    canEnter = false;
                }
