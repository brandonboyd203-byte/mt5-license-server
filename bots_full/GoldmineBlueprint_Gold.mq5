...
void RefreshDailyRiskState() {
    ...
    if(key != dayKey) {
        ...
        Print("Blueprint: Daily equity reset. StartEquity=", DoubleToString(dayStartEquity, 2));
    }
}
...
bool DailyLossExceeded() {
    ...
    if(!dailyLossTripped && lossPct >= maxLossPct) {
        ...
        Print("Blueprint: DAILY LOSS GUARD TRIPPED. Loss=", DoubleToString(lossPct, 2), "% (cap ", DoubleToString(maxLossPct, 2), "%). New entries blocked.");
    }
...
bool DailyProfitTargetReached() {
    ...
    if(!dailyProfitTargetTripped && profitPct >= targetPct) {
        ...
        Print("Blueprint: DAILY PROFIT TARGET REACHED. Profit=", DoubleToString(profitPct, 2), "% (target ", DoubleToString(targetPct, 2), "%). New entries blocked.");
    }
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if(posType == POSITION_TYPE_SELL) {
            ...
            if(sellPips >= bePips && sellPips > 0) {
                ...
                if(UseBreakEven) {
                    Print("[SELL_BE_ARM] Ticket #", ticket, " | Profit=", DoubleToString(sellPips, 1), " pips | Trigger=", bePips, " | Symbol=", posSymbol);
                    double newSL = openPrice;
                    bool needToModify = (currentSL == 0 || newSL < currentSL);
                    if(needToModify) {
                        bool sellBESet = false;
                        ResetLastError();
                        if(trade.PositionModify(ticket, newSL, 0)) {
                            sellBESet = true;
                            Print("[SELL_BE_MOVE_OK] Ticket #", ticket, " | SL -> ", DoubleToString(newSL, posDigits));
                            ...
                            if(trade.PositionModify(ticket, cushionSL, 0)) {
                                sellBESet = true;
                                Print("[SELL_BE_MOVE_OK] Ticket #", ticket, " | SL -> ", DoubleToString(cushionSL, posDigits));
                                Print("*** SELL BE SET (cushion -1 pip) | Ticket #", ticket, " | ", DoubleToString(sellPips, 1), " pips ***");
                            } else if(posDigits >= 3) {
                                cushionSL = NormalizeDouble(openPrice, 2);
                                if(trade.PositionModify(ticket, cushionSL, 0)) {
                                    sellBESet = true;
                                    Print("[SELL_BE_MOVE_OK] Ticket #", ticket, " | SL -> ", DoubleToString(cushionSL, 2));
                        ...
                        if(!sellBESet) {
                            ...
                            Print("[SELL_BE_MOVE_FAIL] Ticket #", ticket, " | Err=", err, " | Symbol=", posSymbol, " | Profit=", DoubleToString(sellPips, 1), " pips");
                        }
                    }
                }
            }
                ...
                if(closeVolume >= minLot && remainingVolume >= minLot) {
                    ...
                    for(int retry = 0; retry < 3; retry++) {
                        ...
                        if(brokerAccepted && volumeReduced) {
                            ...
                            if(posType == POSITION_TYPE_SELL) {
                                Print("[SELL_TP_SET] Ticket #", ticket, " | level=TP1 | Close=", DoubleToString(closeVolume, volumeDigits), " | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                            }
                            closed = true;
                            break;
                        }
                    ...
                    if(!closed) {
                        ...
                        if(posType == POSITION_TYPE_SELL) {
                            Print("[SELL_TP_FAIL] Ticket #", ticket, " | level=TP1 | Close=", DoubleToString(closeVolume, 2), " | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                        }
                    }
                } else {
                ...
                if(closeVolume >= minLot && remainingVolume >= minLot) {
                    ...
                    for(int retry = 0; retry < 3; retry++) {
                        ...
                        if(trade.PositionClosePartial(ticket, closeVolume)) {
                            ...
                            if(posType == POSITION_TYPE_SELL) {
                                Print("[SELL_TP_SET] Ticket #", ticket, " | level=TP2 | Close=", DoubleToString(closeVolume, 2), " | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                            }
                            closed = true;
                            break;
                        } else {
                    ...
                    if(!closed) {
                        ...
                        if(posType == POSITION_TYPE_SELL) {
                            Print("[SELL_TP_FAIL] Ticket #", ticket, " | level=TP2 | Close=", DoubleToString(closeVolume, 2), " | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                        }
                    }
                }
            }
                ...
                if(closeVolume >= minLot && remainingVolume >= minLot) {
                    ...
                    for(int retry = 0; retry < 3; retry++) {
                        ...
                        if(trade.PositionClosePartial(ticket, closeVolume)) {
                            ...
                            if(posType == POSITION_TYPE_SELL) {
                                Print("[SELL_TP_SET] Ticket #", ticket, " | level=TP3 | Close=", DoubleToString(closeVolume, 2), " | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                            }
                            closed = true;
                            break;
                        } else {
                    ...
                    if(!closed) {
                        ...
                        if(posType == POSITION_TYPE_SELL) {
                            Print("[SELL_TP_FAIL] Ticket #", ticket, " | level=TP3 | Close=", DoubleToString(closeVolume, 2), " | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                        }
                    }
                }
            }
            ...
            if(closeVolume >= minLot && closeVolume <= currentVolume - minLot) {
                ...
                if(remainingVolume >= minLot) {
                    if(trade.PositionClosePartial(ticket, closeVolume)) {
                        ...
                        if(posType == POSITION_TYPE_SELL) {
                            Print("[SELL_TP_SET] Ticket #", ticket, " | level=TP4 | Close=", DoubleToString(closeVolume, 2), " | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                        }
                    } else if(posType == POSITION_TYPE_SELL) {
                        Print("[SELL_TP_FAIL] Ticket #", ticket, " | level=TP4 | Close=", DoubleToString(closeVolume, 2), " | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                    }
                }
            }
        }
                ...
                if(reachedSR) {
                    ...
                    if(closeVolume >= minLot && (currentVolume - closeVolume) >= runnerSize) {
                        if(trade.PositionClosePartial(ticket, closeVolume)) {
                            ...
                            if(posType == POSITION_TYPE_SELL) {
                                Print("[SELL_TP_SET] Ticket #", ticket, " | level=TP4_SR | Close=", DoubleToString(closeVolume, 2), " | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                            }
                            partialCloseLevel[ticketIndex] = 4;
                        } else if(posType == POSITION_TYPE_SELL) {
                            Print("[SELL_TP_FAIL] Ticket #", ticket, " | level=TP4_SR | Close=", DoubleToString(closeVolume, 2), " | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                        }
                    }
                }
            }
        ...
        if(!tp5Hit[ticketIndex] && currentProfitPips >= TP5_Pips && currentProfitPips > 0 && partialCloseLevel[ticketIndex] >= 3 && allowFullClose) {
            if(trade.PositionClose(ticket)) {
                ...
                if(posType == POSITION_TYPE_SELL) {
                    Print("[SELL_TP_SET] Ticket #", ticket, " | level=TP5 | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                }
            } else {
                Print("ERROR: Failed to close TP5 runner. Error: ", trade.ResultRetcodeDescription());
                if(posType == POSITION_TYPE_SELL) {
                    Print("[SELL_TP_FAIL] Ticket #", ticket, " | level=TP5 | Profit=", DoubleToString(currentProfitPips, 1), " pips");
                }
            }
        }
