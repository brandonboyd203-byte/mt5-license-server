//+------------------------------------------------------------------+
//|                         GoldmineNexus_Silver_VPS.mq5              |
//|     VPS: Restart-safe. Adopts existing positions on init.        |
//|          Goldmine Nexus – Silver (XAGUSD only)                     |
//|     FVG + OB + Session sweeps + BOS/CHoCH | BE/TP fixed 0.01 pip   |
//+------------------------------------------------------------------+
#property copyright "Goldmine Nexus"
#property link      ""
#property version   "1.00"
#property description "Goldmine Nexus – Silver. Attach to XAGUSD only. BE/TP 1 pip = 0.01."

#define PLUG_SYMBOL_SILVER
...
#ifndef PLUG_SYMBOL_SILVER
input double BreakEvenPips = 25.0;            // Move to BE at this many pips (Gold) – max when UseDynamicBE
#endif
input double BreakEvenPips_Silver = 25.0;    // Move to BE at this many pips (Silver) – used when volatility switch off
...
#endif
input double DynamicSL_MaxPips_Silver = 60.0; // Maximum dynamic SL (pips) - allows structure/spikes
...
input double StructureTrail_BufferPips = 40.0; // Buffer beyond swing (pips)
...
input group "=== General ==="
input int MagicNumber = 124004;              // Magic number (Nexus Silver - unique per EA)
input string TradeComment = "Goldmine Nexus – Silver";  // Trade comment
...
bool ValidateLicenseRemote(string eaNameOverride = "") {
    ...
    string eaName = (StringLen(eaNameOverride) > 0 ? eaNameOverride : "Goldmine Nexus - Silver");
...
//+------------------------------------------------------------------+
bool CheckLicense() {
    Print("Nexus Silver VPS runtime: license checks bypassed for VDS deployment.");
    return true;
}
...
int OnInit() {
    ...
    if(adopted > 0) Print("Goldmine Nexus Silver VPS: Adopted ", adopted, " existing position(s) for BE/TP management.");
...
bool DailyLossExceeded() {
    ...
    if(!dailyLossTripped && lossPct >= maxLossPct) {
        ...
        Print("Nexus Silver VPS: DAILY LOSS GUARD TRIPPED. Loss=", DoubleToString(lossPct, 2), "% (cap ", DoubleToString(maxLossPct, 2), "%). New entries blocked.");
    }
...
bool DailyProfitTargetReached() {
    ...
    if(!dailyProfitTargetTripped && profitPct >= targetPct) {
        ...
        Print("Nexus Silver VPS: DAILY PROFIT TARGET REACHED. Profit=", DoubleToString(profitPct, 2), "% (target ", DoubleToString(targetPct, 2), "%). New entries blocked.");
    }
...
double CalculateDynamicSL(bool isBuy, double entryPrice, double zoneBottom, double zoneTop, int confluenceCount = 1) {
    ...
    // Safety: Gold absolute cap 80 pips (lot sizing uses this SL from entry so risk stays constant)
    if(isGold && slPips > 80.0) {
        Print("  [SAFETY] Gold SL capped at 80 pips (was ", DoubleToString(slPips, 1), " pips)");
        slPips = 80.0;
    }
...
void ManagePositions() {
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if(UseBreakEven && currentProfitPips >= bePips && currentProfitPips > 0) {
            double beTol = (isSilver ? 0.01 : posPipValue);
        ...
        // This is a hard safety net for Silver/Nexus cases where type/trigger state can desync.
        if(UseBreakEven) {
            double infPip = (isSilver ? 0.01 : (isGold ? 0.1 : posPipValue));
