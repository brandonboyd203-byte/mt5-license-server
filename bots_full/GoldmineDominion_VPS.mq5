...
int OnInit() {
    ...
    for(int ai = PositionsTotal() - 1; ai >= 0; ai--) {
        ...
        double profitPips = (position.Type() == POSITION_TYPE_BUY) ? (currentPrice - openPrice) / pv : (openPrice - currentPrice) / pv;
        int idx = GetTicketIndex(ticket);
        EnsureArrays(idx);
        if(idx >= 0 && idx < ArraySize(tp1Hit)) {
            if(originalVolume[idx] == 0) originalVolume[idx] = vol;
            double bePips = isSilver ? BreakEvenPips_Silver : BreakEvenPips;
            if(profitPips >= bePips) tp1Hit[idx] = true;
            adopted++;
        }
    }
    if(adopted > 0) Print("Goldmine Dominion VPS: Adopted ", adopted, " existing position(s) for BE/TP management.");
