    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--) {
        ...
        if(UseBreakEven && profitPips > 0) {
            ...
            if(!alreadyBE) {
                ...
                if(bePips > 0 && profitPips >= bePips) {
                    ...
                    if(!isBuy) Print("Fresh SELL BE telemetry #", ticket, " pips=", profitPips, " bePips=", bePips, " curSL=", currentSL, " newSL=", newSL, " need=", needModify);
                    if(needModify) {
                        if(trade.PositionModify(ticket, newSL, position.TakeProfit()))
                            Print("Fresh BE set ", isBuy ? "BUY" : "SELL", " #", ticket);
                        else {
                            // Some brokers require min distance: try 1, 5, then 10 pips in our favor
                            ...
                            if(trade.PositionModify(ticket, buf1, position.TakeProfit()))
                                Print("Fresh BE set (1 pip buffer) ", isBuy ? "BUY" : "SELL", " #", ticket);
                            else if(trade.PositionModify(ticket, buf5, position.TakeProfit()))
                                Print("Fresh BE set (5 pip buffer) ", isBuy ? "BUY" : "SELL", " #", ticket);
                            else if(trade.PositionModify(ticket, buf10, position.TakeProfit()))
        ...
        if(StringFind(position.Comment(), "_SCALP") >= 0) {
                ...
                if(!scalpAlreadyBE) {
                    ...
                    if(need) {
                        if(trade.PositionModify(ticket, newSL, position.TakeProfit()))
                            ...
                            if(trade.PositionModify(ticket, b1, position.TakeProfit()))
                                Print(revScalp ? "Fresh REV SCALP BE set (1 pip buffer) #" : "Fresh SCALP BE set (1 pip buffer) #", ticket);
                            else if(trade.PositionModify(ticket, b5, position.TakeProfit()))
                                ...
                                if(trade.PositionModify(ticket, b10, position.TakeProfit()))
            ...
            if(UseBreakEven && profitPips >= REV_BEPips) {
                ...
                if(stillNoBE) {
                    ...
                    double b5 = isBuy ? NormalizeDouble(openPrice + 5.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 5.0 * pipValue, symbolDigits);
                    double b10 = isBuy ? NormalizeDouble(openPrice + 10.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 10.0 * pipValue, symbolDigits);
                    if(trade.PositionModify(ticket, ns, position.TakeProfit()))
                        Print("Fresh SCALP catch-all BE set #", ticket);
                    else if(trade.PositionModify(ticket, b1, position.TakeProfit()))
                        Print("Fresh SCALP catch-all BE (1 pip) #", ticket);
                    else if(trade.PositionModify(ticket, b5, position.TakeProfit()))
                        Print("Fresh SCALP catch-all BE (5 pip) #", ticket);
                    else if(trade.PositionModify(ticket, b10, position.TakeProfit()))
        ...
        if(UseBreakEven && profitPips > 0) {
            ...
            if(bePipsOther > 0 && profitPips >= bePipsOther) {
                ...
                if(!alreadyBE2) {
                    ...
                    if(need2) {
                        if(trade.PositionModify(ticket, newSL2, position.TakeProfit()))
        ...
        if(UseBreakEven && profitPips >= REV_BEPips) {
            ...
            if(stillNoBE) {
                ...
                double b5 = isBuy ? NormalizeDouble(openPrice + 5.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 5.0 * pipValue, symbolDigits);
                double b10 = isBuy ? NormalizeDouble(openPrice + 10.0 * pipValue, symbolDigits) : NormalizeDouble(openPrice - 10.0 * pipValue, symbolDigits);
                if(trade.PositionModify(ticket, newSLEnd, position.TakeProfit()))
